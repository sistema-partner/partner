...
export default function ConfirmDialog({
  ...
  const palette = {
    ...
    info: {
      ...
      confirmBg: '!bg-indigo-600 hover:!bg-indigo-700 focus:!ring-indigo-500/50',
    }
  }[variant] || {};
      ...
      <form onSubmit={handleConfirm} className="p-0">
        <div className="bg-white/85 dark:bg-gray-800/85 backdrop-blur-md border border-gray-200 dark:border-gray-700 rounded-lg p-6">
          <div className="flex items-start gap-3">
            <div className="mt-1">
              <AlertTriangle className={`h-6 w-6 ${palette.icon}`} />
            </div>
            <div>
          ...
          <div className="mt-6 flex justify-end gap-3">
            <SecondaryButton type="button" ref={cancelButtonRef} onClick={onClose}>
              {cancelLabel}
            </SecondaryButton>
            <PrimaryButton type="submit" className={palette.confirmBg}>
              {confirmLabel}
            </PrimaryButton>
          </div>
        </div>
